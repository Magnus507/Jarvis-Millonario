...
from core.dispatch_registry import DispatchRegistry
...
class Task:
    ...
    dispatch_id: int | None = field(compare=False, default=None)
...
class TaskQueue:
    def __init__(self, max_concurrent: int = 1):
        ...
        self._executor       = None  
        self._dispatches     = DispatchRegistry()
    ...
    def submit(
        ...
        dispatch_id = self._dispatches.register(f"agent_task:{task_id}", goal)
        task    = Task(
        ...
        try:
            ...
            with self._lock:
                if task.cancel_flag.is_set():
                    ...
                    task.result = result
                    if task.dispatch_id:
                        self._dispatches.update(task.dispatch_id, "completed", summary=str(result)[:300], result=str(result))
                self._active_count -= 1
            ...
            with self._lock:
                ...
                task.error  = str(e)
                if task.dispatch_id:
                    self._dispatches.update(task.dispatch_id, "failed", summary=str(e)[:300], result=str(e))
                self._active_count -= 1
