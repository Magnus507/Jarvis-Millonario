...
import { factions, stickers, users, territories } from './mockDb';
  ...
  return (
    <div className="container py-20 text-center">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <span className="neon-text mb-4" style={{display: 'block', fontWeight: 'bold', letterSpacing: '0.1em', fontSize: '0.875rem'}}>LA GUERRA POR LAS PROVINCIAS HA COMENZADO</span>
...
const LoginView = ({ onLogin }) => {
  ...
  const handleLogin = () => {
    ...
    if (user) {
      ...
      setError('Credenciales inválidas. Usa rider@faccion.com o admin@jarvisin.com (pass: 123)');
    }
  };
    ...
    <div className="container auth-container">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="auth-card">
        <GlassCard>
          ...
          {error && <div style={{color: '#EF4444', fontSize: '0.75rem', marginBottom: '1rem', textAlign: 'center'}}>{error}</div>}
          
          <InputField label="Identificación (Email)" icon={Mail} type="email" placeholder="rider@faccion.com" value={email} onChange={e => setEmail(e.target.value)} />
          <InputField label="Código de Acceso" icon={Lock} type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} />
          ...
          <button onClick={handleLogin} className="btn btn-primary mt-6" style={{width: '100%'}}>Sincronizar</button>
          
          <div className="text-center mt-4 text-muted" style={{fontSize: '0.75rem'}}>
            <p><strong>Cuentas de Prueba:</strong></p>
            <p>Rider: rider@faccion.com / 123</p>
            <p>Admin: admin@jarvisin.com / 123</p>
          </div>
        </GlassCard>
      </motion.div>
    </div>
...
const DashboardView = ({ currentUser }) => {
  if (!currentUser || currentUser.role !== 'rider') {
    return <div className="container py-20 text-center text-xl text-muted">Acceso denegado. <br/><Link to="/login" className="text-primary">Autentícate como Rider</Link></div>;
  }

  const position = [8.9824, -79.5199]; 
  const userFaction = factions.find(f => f.id === currentUser.faction_id) || factions[0];
      ...
      <div className="grid grid-cols-1 md-grid-cols-3 gap-8">
        ...
        <div>
          ...
          <GlassCard>
            ...
            <button className="btn btn-primary" style={{width: '100%'}}>Iniciar Escaneo (Cámara)</button>
...
const AdminView = ({ currentUser }) => {
  if (!currentUser || currentUser.role !== 'admin') {
    return <div className="container py-20 text-center text-xl text-muted">Acceso denegado. <br/><Link to="/login" className="text-primary">Autentícate como Admin</Link></div>;
  }
        ...
        <GlassCard className="md-col-span-2">
          ...
          <div className="flex-col gap-4">
            {[
              ...
              <div key={i} className="flex items-center justify-between mb-4" style={{padding: '1rem', background: 'rgba(255,255,255,0.02)', borderLeft: log.points === 'FLAGGED' ? '3px solid #EF4444' : '3px solid #10B981', borderRadius: '4px'}}>
        ...
        <div className="container flex items-center justify-between">
          ...
          <div className="nav-links">
            ...
            {currentUser?.role === 'rider' && (
              <Link to="/dashboard" className={`nav-link ${location.pathname === '/dashboard' ? 'active' : ''}`}>Radar Táctico</Link>
            )}
            ...
            {!currentUser ? (
              <Link to="/login" className="btn btn-primary" style={{padding: '0.5rem 1.5rem', fontSize: '0.875rem'}}>Conectar</Link>
            ) : (
              <button onClick={handleLogout} className="btn btn-outline" style={{padding: '0.5rem 1.5rem', fontSize: '0.875rem'}}>Desconectar</button>
            )}
          </div>
        </div>
      </nav>
      ...
      <main style={{minHeight: '80vh'}}>
        <AnimatePresence mode="wait">
          <Routes location={location} key={location.pathname}>
            ...
            <Route path="/login" element={<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}><LoginView onLogin={setCurrentUser} /></motion.div>} />
            <Route path="/dashboard" element={<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}><DashboardView currentUser={currentUser} /></motion.div>} />
            <Route path="/admin" element={<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><AdminView currentUser={currentUser} /></motion.div>} />
          </Routes>
        </AnimatePresence>
      </main>
